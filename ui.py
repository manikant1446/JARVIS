...
class AudioDeviceOverlay(_HudOverlay):
    ...
    def __init__(self, parent=None):
        ...
        def _populate_box(box: QComboBox, kind: str, current: str, refresh: bool = False):
            box.clear()
            box.addItem(DEFAULT_LABEL, "")
            devs = list_devices(kind, refresh=refresh)
            for name in devs:
                box.addItem(name, name)
            idx = box.findData(current) if current else -1
            if idx >= 0:
                box.setCurrentIndex(idx)
            else:
                # If no specific device saved, prefer an active Bluetooth headset if connected
                bt_idx = -1
                for i in range(box.count()):
                    item_text = box.itemText(i).lower()
                    if any(k in item_text for k in ("wireless", "bluetooth", "bullets", "airpods", "buds", "headset", "headphones")):
                        bt_idx = i
                        break
                if bt_idx >= 0 and not current:
                    box.setCurrentIndex(bt_idx)
                elif current:
        ...
        def _row(label: str, kind: str, current: str) -> QComboBox:
            ...
            _populate_box(box, kind, current, refresh=True)
            lay.addWidget(box)
            return box
        ...
        def _on_refresh():
            from memory.config_manager import get_input_device, get_output_device
            cur_in = self._in_box.currentData() or get_input_device()
            cur_out = self._out_box.currentData() or get_output_device()
            _populate_box(self._in_box, "input", cur_in, refresh=True)
            _populate_box(self._out_box, "output", cur_out, refresh=True)

        ref_btn.clicked.connect(_on_refresh)
    ...
    def _create_desktop_shortcut(self):
        ...
        try:
            ...
            if _os == "Windows":
                ...
                lnk      = str(desktop / f"{asst_name}.lnk")
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    f'  <key>CFBundleExecutable</key><string>{asst_name}</string>\n'
                    f'  <key>CFBundleIdentifier</key><string>com.{asst_name.lower()}.assistant</string>\n'
                    f'  <key>CFBundleName</key><string>{asst_name}</string>\n'
            ...
            # ── Linux — .desktop file (Terminal=false, no console) ────────────
                ...
                desk = desktop / f"{asst_name}.desktop"
                desk.write_text(
                    "[Desktop Entry]\n"
                    f"Name={asst_name}\n"
                    f"Exec={python} {script}\n"
            ...
            self._log.append_log(f"SYS: Desktop shortcut created: {asst_name}")
        except Exception as e:
