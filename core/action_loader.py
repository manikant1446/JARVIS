...
def _validate_tool(tool: dict, filename: str) -> ActionRecord:
    """Returns an ActionRecord for a single tool dict; .valid=False + .error set on any problem. Never raises."""
    if not isinstance(tool, dict):
        return ActionRecord(name=Path(filename).stem, file=filename,
                            error="Tool must be a dictionary.")
...
def discover_actions(actions_dir: Path, reserved_names: set[str] | None = None,
    ...
    for path in files:
        ...
        try:
            ...
            tool_single = getattr(module, "TOOL", None)
            tool_multi = getattr(module, "TOOLS", None)

            if tool_single is None and tool_multi is None:
                continue   # not an action file — a helper/capture-only module

            raw_tools: list[dict] = []
            if isinstance(tool_single, dict):
                raw_tools.append(tool_single)
            if isinstance(tool_multi, (list, tuple)):
                raw_tools.extend(t for t in tool_multi if isinstance(t, dict))

            if not raw_tools:
                rec = ActionRecord(name=path.stem, file=path.name,
                                   error="TOOL/TOOLS defined but contained no valid dictionary.")
                all_records.append(rec)
                logger(f"Action rejected: {path.name} — {rec.error}")
                continue

            for t_dict in raw_tools:
                rec = _validate_tool(t_dict, path.name)
