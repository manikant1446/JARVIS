...
def main() -> None:
    ...
    # Ensure we are inside a virtual environment or handle PEP 668 gracefully
    in_venv = sys.prefix != sys.base_prefix
    py_exec = sys.executable

    if not in_venv:
        venv_dir = Path(__file__).resolve().parent / ".venv"
        venv_py = venv_dir / "bin" / "python" if OS != "Windows" else venv_dir / "Scripts" / "python.exe"
    ...
    _run("Installing Playwright browsers (chromium + firefox)…",
         [py_exec, "-m", "playwright", "install", "chromium", "firefox"])
    ...
    if not in_venv:
        if OS == "Windows":
            print("   1) Launch it:  .venv\\Scripts\\activate && python main.py")
        else:
            print("   1) Launch it:  source .venv/bin/activate && python main.py")
