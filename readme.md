...
classDiagram
    class PermissionEngine {
        ...
        +execute_with_permission(level, title, detail, action_fn)
    }
    class Level0_READ {
        +Info Queries (Battery, WiFi, Stocks)
        +Auto-execute immediately
        +Zero confirmation
    }
    class Level1_SAFE {
        +Local Reversible (Volume, Brightness, Music)
        +Auto-execute immediately
        +Pushed to Undo Stack
    }
    class Level2_EXTERNAL {
        +External Comms (iMessage, WhatsApp, Email)
        +Confirmation Banner + Voice Gate
        +Previews Recipient + Body
    }
    class Level3_DESTRUCTIVE {
        +Delete Files, Empty Trash, Power, Shell
        +Strict Confirmation Gate
        +Audit Log Recording
    }
    PermissionEngine --> Level0_READ
    PermissionEngine --> Level1_SAFE
    PermissionEngine --> Level2_EXTERNAL
